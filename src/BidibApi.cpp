...
bool BidibApi::selfTestTrack() {
	...
	setAllTurnoutsState(Turnout::straightOn);
	printf(
			"You can place the loc now on segment A2 in direction TA2 and start ... \n");
	...
	if (getNumberOfLocs() > 1) {
		printf(
				"To much locs on track, please place only ONE loc on segment A2 in direction TA2 and start again.\n");
		return false;
	}
...
void BidibApi::setLocFunction(int locID, locFunction::locFunctionID ID,
		bool functionState) {
...
void BidibApi::setTurnoutState(Turnout::turnoutID turnID,
...
bool BidibApi::isConnected() {
	if ((bidibMessageHandler.isConnected() == 0)
			|| (bidibMessageHandler.isConnected() == -1)) {
		return false;
	}
	if (bidibMessageHandler.isConnected() == 1) {
		return true;
