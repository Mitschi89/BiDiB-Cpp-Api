...
bool BiDiBMessageHandler::initBidib() {
	printf("Initializing BiDiB-Protocol. ");
	fflush(stdout);
...
void BiDiBMessageHandler::sendMessage(unsigned char* message) {
	...
	for (int i = 0; i < length + 1; i++) {
		if ((message[i] == BIDIB_PKT_MAGIC)
				|| (message[i] == BIDIB_PKT_ESCAPE)) {
...
char* BiDiBMessageHandler::getMessageType(int type) {
	...
	case MSG_BM_MULTIPLE:
		return "MSG_BM_MULTIPLE ";		//liefert StartAdresse der Gleise,
										//Anzahl der folgenden bits für belegte Gleise,
										//bitweise kodierte belegte/frei Gleise
	case MSG_BM_OCC:
		return "MSG_BM_OCC"; 	//liefert Adresse des Gleises, dass belegt ist
	case MSG_BM_FREE:
		return "MSG_BM_FREE "; //liefert Adresse des Gleises, dass wieder frei ist
	case MSG_BM_ADDRESS:
		return "MSG_BM_ADDRESS "; //liefert Adresse des Gleises, kurze Adresse, lange Adress der loc
...
int BiDiBMessageHandler::processMessage(unsigned char *message, int length) {
	...
	do {
		...
		case MSG_SYS_MAGIC:
		case MSG_BOOST_DIAGNOSTIC:
			if (SHOWDIAGNOSTIC & SHOWSYSMESSAGES) {
				printReceiveMessage(msg);
			}
			processOther(msg);
			break;
...
int BiDiBMessageHandler::processTurnoutStateMessage(unsigned char* message) {
	...
	if (message[3 + messageOffset] == MSG_LC_STAT) {
		...
		for (int i = 0; i < MAXNUMBEROFSEGEMENTSWITHLOC; i++) {
			locs[i].stateMaschine.turnouts[turnID].turnDir =
					turnouts[turnID].turnDir;
		}
...
int BiDiBMessageHandler::processNodeTabMessage(unsigned char* message) {
	...
	if (message[3] == MSG_NODETAB_COUNT) {
		...
		if (nodeCount == 0) {
			sendNodeTabMessage();
			printf("another try to get NodeTab...\n");
			fflush(stdout);
		} else {
			if (nodeCount != NODECOUNT) {
				printf(
						"NOT ALL NODES AVAILABLE. CHECK CABLE CONNECTION OR PRESS RESET!\n");
			}
		}
	}
	...
	if (message[3] == MSG_NODETAB) {
		...
		switch (ID) {
		case 0xDE:
			gbmMasterID = entryNumber;
			break;
		case 0x76:
			oneControlID = entryNumber;
			break;
		case 0x73:
			oneOcID = entryNumber;
			break;
		default:
			printf(
					"Node not recognized. Make sure Node is specified in code. See processNodeTabMessage().\n");
		}
	}
	...
	if (message[3 + messageOffset] == MSG_NODE_NA) {
		...
		if ((entryNumber != gbmMasterID) && (entryNumber != oneControlID)
				&& (entryNumber != oneOcID)) {
...
int BiDiBMessageHandler::processErrorMessage(unsigned char* message) {
	...
	case BIDIB_ERR_SUBTIME:
		printf("SubNode Message Timeout ERROR\n");
		break;
	default:
...
void BiDiBMessageHandler::sendDriveMessage(int locID) {
	...
	unsigned char message[] = { 12, gbmMasterID, ++msgNum[gbmMasterID],
	MSG_CS_DRIVE, locs[locID].id, //address low of loc
			...
			functions, 0, 0, 0 //functions active
			};

	sendMessage(message);
}

void BiDiBMessageHandler::sendDriveMessage(int locID, int speed,
		bool direction) {
	...
	if (locs[locID].stateMaschine.direction
			== locs[locID].stateMaschine.logicalDirection) {
...
void BiDiBMessageHandler::sendFunctionStateMessage(int locID,
		int functionNumber, bool functionState) {
...
void BiDiBMessageHandler::sendFeatureMessage(int node) {
	if (node == gbmMasterID) {
		unsigned char message[] = { 3, gbmMasterID, ++msgNum[node],
		MSG_FEATURE_GETALL };
		sendMessage(message);

		usleep(1000 * 1000);
		for (int i = 0; i < featureCount; i++) {
			unsigned char message[] = { 3, node, ++msgNum[node],
			MSG_FEATURE_GETNEXT };
			sendMessage(message);
			usleep(50 * 1000);
		}
	} else {
		unsigned char message[] = { 4, node, gbmMasterID, ++msgNum[node],
		MSG_FEATURE_GETALL };
		sendMessage(message);
...
void BiDiBMessageHandler::sendNodeTabMessage() {
	unsigned char message[] = { 3, gbmMasterID, ++msgNum[gbmMasterID],
	MSG_NODETAB_GETALL };
	sendMessage(message);
	usleep(2000 * 1000);
...
void BiDiBMessageHandler::sendTurnMessage(int turnID, bool direction) {
	...
	unsigned char onMessage[] = { 8, oneControlID, gbmMasterID,
			++msgNum[oneControlID],
			MSG_LC_OUTPUT, 0x00, address, 1 };

	unsigned char offMessage[] = { 8, oneControlID, gbmMasterID,
			++msgNum[oneControlID],
			MSG_LC_OUTPUT, 0x00, address, 0 };
...
void BiDiBMessageHandler::sendBoostOnMessage() {
	unsigned char message[] = { 4, gbmMasterID, ++msgNum[gbmMasterID],
	MSG_BOOST_ON, 0x00 };
	sendMessage(message);
}
...
void BiDiBMessageHandler::sendGetLocsMessage() {
	unsigned char message[] = { 5, gbmMasterID, ++msgNum[gbmMasterID],
	MSG_BM_ADDR_GET_RANGE, 0x00, 31 };
	sendMessage(message);
}

void BiDiBMessageHandler::sendGetSwitchesMessage() {
	unsigned char message[] = { 6, oneOcID, gbmMasterID, ++msgNum[oneOcID],
	MSG_BM_ADDR_GET_RANGE, 0x00, 24 };
	sendMessage(message);
}
...
void BiDiBMessageHandler::sendMakro(int turnID, bool direction) {
	...
	unsigned char message[] = { 6, oneControlID, gbmMasterID,
			++msgNum[oneControlID],
			MSG_LC_MACRO_HANDLE, index, 1 };
	sendMessage(message);
}
...
void BiDiBMessageHandler::sendMirrorMessage(unsigned char* message) {
	...
	switch (type) {
	case MSG_BM_MULTIPLE:
		mirrorType = MSG_BM_MIRROR_MULTIPLE;
		break;
	case MSG_BM_OCC:
		mirrorType = MSG_BM_MIRROR_OCC;
		break;
	case MSG_BM_FREE:
		mirrorType = MSG_BM_MIRROR_FREE;
		break;
	default:
		return;
	}
