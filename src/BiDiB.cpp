...
int locCount = 0;
int speed = 50;
Segment::segmentID criticalState[MAXNUMBEROFSEGEMENTSWITHLOC] = {
		Segment::DEFAULT };
...
void handlePredictions(BidibApi *api) {
	...
	for (int locID = 0; locID < locCount; locID++) {
		...
		if ((api->getCurrentState(locID) == Segment::A1)
				|| (api->getCurrentState(locID) == Segment::A2)) {
			if (!(api->isLocOnPosition(locID, Segment::TA1))
					&& !(api->isLocOnPosition(locID, Segment::TA2))) {
				if ((criticalState[locID] != Segment::A1)
						&& (criticalState[locID] != Segment::A2)) {
					if (api->getLocLogicalDirection(locID) == false) {
						api->setLocSpeed(locID, speed,
								!api->getLocDirection(locID));
		...
		for (int nextLocID = locID + 1; nextLocID < locCount; nextLocID++) {
			...
			if (api->getPredictedNextState(locID)
					== api->getPredictedNextState(nextLocID)) {
				if ((api->getCurrentState(locID) != criticalState[locID])
						&& (api->getCurrentState(nextLocID)
								!= criticalState[nextLocID])) {

					api->setLocSpeed(locID, speed,
							!api->getLocDirection(locID));
					api->setLocSpeed(nextLocID, speed,
							!api->getLocDirection(nextLocID));
					criticalState[locID] = api->getCurrentState(locID);
					criticalState[nextLocID] = api->getCurrentState(nextLocID);
					printf(
							"Equal Predicted State: loc: %d and loc: %d at position: %s\n",
							locID, nextLocID,
							Segment::convertSegmentName(
									api->getPredictedNextState(locID)));
					fflush(stdout);
				}
			}
			if (api->getPredictedNextState(locID)
					== api->getCurrentState(nextLocID)) {
				if (api->getLocLogicalDirection(locID)
						== api->getLocLogicalDirection(nextLocID)) {
					api->setLocSpeed(locID, speed / 2,
							api->getLocDirection(locID));
				} else {
					if ((api->getCurrentState(locID) != criticalState[locID])
							&& (api->getCurrentState(nextLocID)
						...
						api->setLocSpeed(nextLocID, speed,
								!api->getLocDirection(nextLocID));
						criticalState[locID] = api->getCurrentState(locID);
						criticalState[nextLocID] = api->getCurrentState(
								nextLocID);
						printf(
								"Predicted State is current State: loc: %d and loc: %d at position: %s\n",
								locID, nextLocID,
								Segment::convertSegmentName(
										api->getPredictedNextState(locID)));
						fflush(stdout);
			...
			if (api->getCurrentState(locID)
					== api->getPredictedNextState(nextLocID)) {
				if (api->getLocLogicalDirection(locID)
						== api->getLocLogicalDirection(nextLocID)) {
					api->setLocSpeed(nextLocID, speed / 2,
							api->getLocDirection(nextLocID));
				} else {
					if ((api->getCurrentState(locID) != criticalState[locID])
							&& (api->getCurrentState(nextLocID)
						...
						criticalState[nextLocID] = api->getCurrentState(
								nextLocID);
						printf(
								"current State is Predicted State: loc: %d and loc: %d at position: %s\n",
								locID, nextLocID,
								Segment::convertSegmentName(
										api->getCurrentState(locID)));
						fflush(stdout);
					}
				}
			}
				...
				if (api->getLocLogicalDirection(locID)
					...
					if ((api->getCurrentState(locID) != criticalState[locID])
						...
						criticalState[nextLocID] = api->getCurrentState(
								nextLocID);
						printf(
								"current state is current state: loc: %d and loc: %d at position: %s\n",
								locID, nextLocID,
								Segment::convertSegmentName(
										api->getPredictedNextState(locID)));
						fflush(stdout);
...
void switchRandomTurn(BidibApi *api) {
	...
	do {
		...
		for (int locID = 0; locID < locCount; locID++) {
			if ((api->isLocOnPosition(locID, segID))
					|| (api->getPredictedNextState(locID))
					|| (api->getCurrentState(locID))) {
				turnOk = false;
			}
